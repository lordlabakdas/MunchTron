...
class SentenceParser:
    def parse_sentence(self, words):
        ...
        if any(x in words["sentence"].split() for x in weather_json["prefixer"]):
            ...
            print("twitter")
            result = requests.post(
                "http://localhost:5000/twitter", data=json.dumps(words), headers=headers
            )
            return result.text
            ...
            print("yelp")
            result = requests.post(
                "http://localhost:5000/yelp", data=json.dumps(words), headers=headers
            )
            return result.text
            ...
            print("spotify")
            result = requests.post(
                "http://localhost:5000/spotify", data=json.dumps(words), headers=headers
            )
            return result.text
        else:
            print(None)
